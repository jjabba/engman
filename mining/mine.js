    ...
    .then((tupleArray) => {
        return Promise.all(tupleArray.reduce((events, pair) => {
            if (!pair.dbtask) {
                ...
                const pojoEvents = generateChangeEvents(pair.task, pair.dbtask)

                if (pojoEvents.length > 0) {
                    events.push(taskRepo.update(new Task(pair.task)).then(() => {
...
function generateChangeEvents(task, dbTask, taskRepo) {
    t1 = new Task(task)
    t2 = new Task(dbTask)
